...
function saveImage(url) {
  axios
    .get(url, { headers, timeout: 3000 })
    .then((response) => {
      const $ = cheerio.load(response.data);
      const image = $("#screenshot-image").attr("src");

      download
        .image({ url: image, dest: "./images" })
        .then(({ filename }) => {
          checked++;
          success++;
          setTerminalTitle(
            `Checked: ${checked} | Success: ${success} | Not available: ${notAvailable}`
          );

          console.log(
            chalk.green(
              `[ + ] Screenshot with the ID ${url
                .split("/")
                .pop()} saved to ${filename}`
            )
          );
        })
        .catch(() => {
    ...
    .catch((err) => {
      if (err) {
        ...
        if (err.response.status === 403) {
          ...
          console.log(
            chalk.red(
              "[ - ] Cloudflare blocked your IP, use a VPN. Proxies are not supported yet."
            )
          );
          process.exit();
        }
...
inquirer
  ...
  .then((answer) => {
    if (!answer.images_length) {
      ...
      printSlug();
      console.log(
        chalk.green(`Downloading ${imagesLength} images from ${urlPrefix}\n\n`)
      );
